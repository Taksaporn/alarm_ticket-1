    ...
    for item in lst:
        for l in item:
            if l['status'] != 'closed':
                ...
                # delete_activity_query = """DELETE FROM `activities_tickets` WHERE `ticketNo`='{0}'""".format(l['incident_id'])
                # db.insert(delete_activity_query)
                # for t in ticket_info['activity_table']:
                #     date = str(t['datestamp']).split('/')
                #     year = date[2].split(' ')
                #     datetime = year[0]+'-'+date[1]+'-'+date[0]+' '+year[1]
                #
                #     insert_activity_query = """INSERT INTO `activities_tickets`(`ticketNo`, `number`, `datetime`, `operator`, `division`, `description`, `type`) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}')""".format(
                #         l['incident_id'], t['number'], datetime, t['operator'].encode('utf-8'),
                #         t['division'].encode('utf-8'), t['description'].encode('utf-8'), t['type']
                #     )
                #     db.insert(insert_activity_query)
                #     # update_activity_query = """UPDATE `activities_tickets` SET `ticketNo`='{0}',`number`='{1}',`datetime`='{2}',`operator`='{3}',`division`='{4}',`description`='{5}',`type`='{6}' WHERE `ticketNo`='{7}' AND `number`='{8}'""".format(
                #     #     l['incident_id'], t['number'], t['datestamp'], t['operator'].encode('utf-8'), t['division'].encode('utf-8'), t['description'].encode('utf-8'), t['type'], l['incident_id'], t['number']
                #     # )
    lst.remove
