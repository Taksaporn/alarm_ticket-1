...
class TTS(object):
    ...
    def Get_TicketInfo(self, ticketNo):
        ...
        for l in lst_name:
            ...
            lem = str(infoname).split('TagCollection([AdvancedTag(u\'')[1].split('\',')[0]
            if lem == 'textarea':
                # print infoname.all()[0].innerHTML.strip()
                info[l] = infoname.all()[0].innerHTML.strip()
            elif lem == 'input':
                # print str(infoname).split('u\'value\', u\'')[1].split('\')')[0].encode('utf-8')
                info[l] = str(infoname).split('u\'value\', u\'')[1].split('\')')[0].encode('utf-8')
